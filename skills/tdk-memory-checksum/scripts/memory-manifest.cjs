...
var DEFAULT_MANIFEST_PATH = "memory.yaml";
var INDEX_PATH = "memory-index.md";
var CHANGELOG_PATH = "CHANGELOG.md";
var MEMORY_MAP_PATH = "memory-map.canvas";
var TEMPLATE_DIRECTORY = "_templates";
var EXCLUDED_DIRECTORY_NAMES = {
  _deprecated: true,
...
var MAX_YAML_NODES = 20000;
var MAX_YAML_DEPTH = 32;
var MAX_INVENTORY_ENTRIES = 1e5;
...
function parseInvocation(argumentsAfterNode) {
    ...
    for (const argument of rest) {
      if (argument === "--allow-external-root") {
        if (allowExternalRoot)
          fail("--allow-external-root may only be provided once");
  ...
  if (command === "validate") {
    ...
    for (let index = 1;index < rest.length; index += 1) {
      ...
      if (argument === "--manifest") {
        if (manifestProvided || index + 1 >= rest.length || rest[index + 1].startsWith("--"))
          usage();
...
async function lstatOptional(filePath) {
  try {
    return await import_node_fs.promises.lstat(filePath);
  } catch (error) {
...
async function realpathFromNearestExisting(inputPath) {
  let candidate = import_node_path.resolve(inputPath);
  const missingSegments = [];
  while (true) {
    try {
      const physical = await import_node_fs.promises.realpath(candidate);
      return import_node_path.resolve(physical, ...missingSegments.reverse());
    } catch (error) {
      ...
      if (parent === candidate)
        fail(`cannot resolve an existing ancestor for ${inputPath}`);
      missingSegments.push(import_node_path.basename(candidate));
      candidate = parent;
    }
  }
}
function requireContained(parentPath, childPath, subject) {
  const relation = import_node_path.relative(parentPath, childPath);
  if (relation === "" || !relation.startsWith(`..${import_node_path.sep}`) && relation !== ".." && !import_node_path.isAbsolute(relation))
    return;
  fail(`${subject} escapes containment: ${childPath} is not within ${parentPath}`);
}
async function discoverWorkspaceRoot() {
  const cwd = await import_node_fs.promises.realpath(process.cwd());
  let candidate = cwd;
  while (true) {
    const hasGit = await lstatOptional(import_node_path.join(candidate, ".git"));
    const hasSpecify = await lstatOptional(import_node_path.join(candidate, ".specify"));
    if (hasGit !== undefined || hasSpecify !== undefined)
      return candidate;
    const parent = import_node_path.dirname(candidate);
    if (parent === candidate) {
...
async function resolveMemoryRoot(memoryRootArgument, allowExternalRoot) {
  const workspaceRoot = await discoverWorkspaceRoot();
  const lexicalPath = import_node_path.resolve(workspaceRoot, memoryRootArgument);
  if (!allowExternalRoot)
    requireContained(workspaceRoot, lexicalPath, "memory root path");
  const candidate = await realpathFromNearestExisting(lexicalPath);
  if (!allowExternalRoot)
    requireContained(workspaceRoot, candidate, "memory root symlink");
  const info = await lstatOptional(lexicalPath);
  if (info === undefined)
    fail(`memory root does not exist: ${lexicalPath}`);
  let memoryRoot;
  try {
    memoryRoot = await import_node_fs.promises.realpath(lexicalPath);
  } catch (error) {
    fail(`cannot resolve memory root ${lexicalPath}: ${errorMessage(error)}`);
  }
  if (!allowExternalRoot)
    requireContained(workspaceRoot, memoryRoot, "memory root");
  if (!info.isDirectory() && !info.isSymbolicLink())
    fail(`memory root is not a directory: ${lexicalPath}`);
  const physicalInfo = await lstatOptional(memoryRoot);
  if (physicalInfo === undefined || !physicalInfo.isDirectory())
    fail(`memory root is not a directory: ${memoryRoot}`);
  return memoryRoot;
}
function validateRelativePath(relativePath, subject) {
  if (relativePath.length === 0 || relativePath.includes("\x00"))
    fail(`${subject} must be a non-empty relative path`);
...
async function rejectSymlink(entryPath, memoryRoot) {
  let destination;
  try {
    destination = await import_node_fs.promises.realpath(entryPath);
  } catch (error) {
...
async function inspectContainedTarget(memoryRoot, relativePath, subject) {
  const parts = validateRelativePath(relativePath, subject);
  let candidate = memoryRoot;
  for (const part of parts) {
    candidate = import_node_path.join(candidate, part);
    const info2 = await lstatOptional(candidate);
    if (info2 === undefined)
      return { kind: "missing" };
    if (info2.isSymbolicLink())
      await rejectSymlink(candidate, memoryRoot);
  }
...
async function hashRegularFile(filePath) {
  let handle;
  try {
    handle = await import_node_fs.promises.open(filePath, import_node_fs.constants.O_RDONLY | (import_node_fs.constants.O_NOFOLLOW ?? 0));
    const info = await handle.stat();
    if (!info.isFile())
      fail(`not a regular file: ${filePath}`);
    const digest = import_node_crypto.createHash("sha256");
...
async function readManifestSource(filePath) {
  ...
  try {
    ...
    const info = await handle.stat();
    if (!info.isFile())
      fail(`manifest is not a regular file: ${filePath}`);
    if (info.size > MAX_MANIFEST_BYTES)
      fail(`manifest exceeds ${MAX_MANIFEST_BYTES} bytes: ${filePath}`);
...
function rejectUnsupportedYamlNodes(node, state) {
  if (node === null || node === undefined)
    return;
  state.nodes += 1;
  if (state.nodes > MAX_YAML_NODES)
    fail(`manifest exceeds ${MAX_YAML_NODES} YAML nodes`);
  if (state.depth > MAX_YAML_DEPTH)
    fail(`manifest exceeds YAML nesting depth ${MAX_YAML_DEPTH}`);
  if (import_yaml.isAlias(node))
    fail("manifest YAML aliases are not allowed");
  if ("anchor" in node && node.anchor !== undefined)
    fail("manifest YAML anchors are not allowed");
  if (node.tag !== undefined && !node.tag.startsWith("tag:yaml.org,2002:")) {
    fail(`manifest YAML custom tag is not allowed: ${node.tag}`);
  }
  if (import_yaml.isMap(node)) {
  ...
  } else if (import_yaml.isSeq(node)) {
    for (const item of node.items) {
      state.depth += 1;
...
function mapEntries(node, subject) {
  if (!import_yaml.isMap(node))
    fail(`${subject} must be a mapping`);
  const result = new Map;
  for (const pair of node.items) {
    if (!import_yaml.isScalar(pair.key) || typeof pair.key.value !== "string")
      fail(`${subject} keys must be strings`);
    if (result.has(pair.key.value))
      fail(`${subject} contains duplicate key ${pair.key.value}`);
    result.set(pair.key.value, pair.value ?? null);
  }
  return result;
}
function requireExactKeys(entries, required, optional, subject) {
  const allowed = new Set([...required, ...optional]);
  for (const key of entries.keys()) {
    if (!allowed.has(key))
      fail(`${subject} contains unsupported key ${key}`);
  }
  for (const key of required) {
    if (!entries.has(key))
      fail(`${subject} is missing required key ${key}`);
  }
}
function stringValue(node, subject) {
  if (!import_yaml.isScalar(node) || typeof node.value !== "string")
    fail(`${subject} must be a string`);
  return node.value;
}
function requireHash(value, subject) {
  if (!SHA256_PATTERN.test(value))
    fail(`${subject} must be a lowercase 64-character SHA-256 hexadecimal digest`);
  return value;
}
...
function requireNonEmpty(value, subject) {
  if (value.trim().length === 0)
    fail(`${subject} must not be empty`);
  return value;
}
function requireFileInventoryPath(value, subject) {
  ...
  if (value === INDEX_PATH)
    fail(`${subject} must not be ${INDEX_PATH}; it is covered by memory_index_sha256`);
  if (value !== MEMORY_MAP_PATH && !value.endsWith(".md")) {
    fail(`${subject} must name a Markdown note or ${MEMORY_MAP_PATH}`);
  }
  return value;
}
function requireTemplateInventoryPath(value, subject) {
  const parts = validateRelativePath(value, subject);
...
function parseFileReceipt(node, index) {
...
function parseTemplateReceipt(node, index) {
...
function parseReceiptSequence(node, subject, parseReceipt) {
  if (!import_yaml.isSeq(node))
    fail(`${subject} must be a sequence`);
...
async function readManifest(manifestPath) {
  ...
  let document;
  try {
    document = import_yaml.parseDocument(source, {
      customTags: null,
      merge: false,
      prettyErrors: false,
  ...
  const manifest = {
    ...
    memoryIndexSha256: requireHash(stringValue(entries.get("memory_index_sha256"), "manifest.memory_index_sha256"), "manifest.memory_index_sha256"),
    files: parseReceiptSequence(entries.get("files"), "manifest.files", parseFileReceipt),
    templates: entries.has("templates") ? parseReceiptSequence(entries.get("templates"), "manifest.templates", parseTemplateReceipt) : []
  };
  const paths = new Set;
  for (const receipt of [...manifest.files, ...manifest.templates]) {
    if (paths.has(receipt.path))
      fail(`manifest contains duplicate receipt path ${receipt.path}`);
    paths.add(receipt.path);
  }
  return manifest;
}
async function scanFiles(memoryRoot, startingRelativePath, includeFile) {
  const found = [];
  const pending = [
    { path: startingRelativePath === "" ? memoryRoot : import_node_path.join(memoryRoot, ...startingRelativePath.split("/")), relativePath: startingRelativePath }
  ];
  ...
  while (pending.length > 0) {
    ...
    let entries;
    try {
      entries = await import_node_fs.promises.readdir(current.path, { withFileTypes: true });
    } catch (error) {
    ...
    for (const entry of entries) {
      visitedEntries += 1;
      if (visitedEntries > MAX_INVENTORY_ENTRIES)
        fail(`memory inventory exceeds ${MAX_INVENTORY_ENTRIES} entries`);
      const absolutePath = import_node_path.join(current.path, entry.name);
      ...
      if (info === undefined)
        fail(`memory inventory entry disappeared while scanning: ${absolutePath}`);
      if (info.isSymbolicLink())
        await rejectSymlink(absolutePath, memoryRoot);
      if (info.isDirectory()) {
        requireContained(memoryRoot, absolutePath, "memory inventory directory");
        if (EXCLUDED_DIRECTORY_NAMES[entry.name] !== true)
          pending.push({ path: absolutePath, relativePath });
        continue;
      }
      if (info.isFile()) {
...
async function scanManifestFiles(memoryRoot) {
  return scanFiles(memoryRoot, "", (relativePath) => {
    if (relativePath === INDEX_PATH || relativePath === CHANGELOG_PATH || relativePath === MEMORY_MAP_PATH)
      return false;
    return relativePath.endsWith(".md");
  });
}
async function scanTemplateFiles(memoryRoot) {
  const templateRoot = await inspectContainedTarget(memoryRoot, TEMPLATE_DIRECTORY, "template directory");
  if (templateRoot.kind === "missing")
    return [];
  if (templateRoot.kind !== "non-file")
    fail(`template directory is not a directory: ${import_node_path.join(memoryRoot, TEMPLATE_DIRECTORY)}`);
  const templateInfo = await lstatOptional(import_node_path.join(memoryRoot, TEMPLATE_DIRECTORY));
...
async function hashContainedFile(memoryRoot, relativePath, subject) {
  const target = await inspectContainedTarget(memoryRoot, relativePath, subject);
  if (target.kind !== "file")
    return;
  return hashRegularFile(target.path);
}
async function validate(memoryRootArgument, manifestRelativePath, allowExternalRoot) {
  ...
  if (manifestTarget.kind !== "file")
    fail(`manifest not found: ${import_node_path.join(memoryRoot, ...validateRelativePath(manifestRelativePath, "manifest path"))}`);
  ...
  for (const relativePath of [...diskTemplates].sort((left, right) => left.localeCompare(right))) {
    ...
    if (actual === undefined)
      fail(`template disappeared while validating: ${relativePath}`);
...
async function hash(memoryRootArgument, relativePath, allowExternalRoot) {
  const memoryRoot = await resolveMemoryRoot(memoryRootArgument, allowExternalRoot);
  const target = await inspectContainedTarget(memoryRoot, relativePath, "hash path");
  if (target.kind !== "file")
    fail(`file not found: ${import_node_path.join(memoryRoot, ...validateRelativePath(relativePath, "hash path"))}`);
